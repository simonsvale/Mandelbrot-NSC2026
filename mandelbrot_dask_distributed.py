...
if __name__ == "__main__":
    ...
    workers = 2 # This needs to be manually set, as psutil only graps the local number of cores.
    print(f"Number of workers: {workers}")

    N_chunks = 256 # The optimal number of chunks, chunk size: (8192x32)
    ...
    time_list = []
    for _ in range(run_count):
        _, exec_time = mandelbrot_dask(N, x_interval, y_interval, max_iter, N_chunks)
        time_list.append(exec_time)
    Tp = statistics.median(time_list)
